...
class SAGE_NeighSampler(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout, batchnorm=True):
        ...
        for i in range(num_layers - 2):
            ...
            if self.batchnorm:
                self.bns.append(torch.nn.BatchNorm1d(hidden_channels))
        self.convs.append(SAGEConv(hidden_channels, int(hidden_channels/2)))

        self.dropout = dropout

        self.fc = nn.Linear(int(hidden_channels/2), out_channels)
    ...
    def to_embedding(self, x_all, layer_loader, device):
        pbar = tqdm(total=x_all.size(0) * self.num_layers, ncols=80)
        pbar.set_description('Evaluating')
